...
public class ImageViewerDialog extends JDialog {
    ...
    private void commonInit() {
        ...
        addComponentListener(new ComponentAdapter() {
            @Override public void componentResized(ComponentEvent e) {
                if (fitMode) fitToWindow(); else renderZoom();
            }
        });
        ...
        view.addMouseListener(new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) { start[0] = e.getPoint(); view.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); }
            @Override public void mouseReleased(MouseEvent e) { view.setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR)); }
        });
        view.addMouseMotionListener(new MouseMotionAdapter() {
            @Override public void mouseDragged(MouseEvent e) {
                ...
                if (p.x + vp.getWidth() > view.getWidth()) p.x = Math.max(0, view.getWidth()-vp.getWidth());
                if (p.y + vp.getHeight() > view.getHeight()) p.y = Math.max(0, view.getHeight()-vp.getHeight());
                vp.setViewPosition(p);
            }
        });

        // 단축키
        getRootPane().registerKeyboardAction(e -> zoomIn(),   KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, InputEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW);
        getRootPane().registerKeyboardAction(e -> zoomOut(),  KeyStroke.getKeyStroke(KeyEvent.VK_MINUS,  InputEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW);
        getRootPane().registerKeyboardAction(e -> zoom100(),  KeyStroke.getKeyStroke(KeyEvent.VK_0,      InputEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW);
        getRootPane().registerKeyboardAction(e -> toggleFit(),KeyStroke.getKeyStroke(KeyEvent.VK_F, 0), JComponent.WHEN_IN_FOCUSED_WINDOW);
    }
    ...
    private JComponent buildToolbar() {
        ...
        var title = new JLabel(displayTitle == null
                ? (fullPath == null ? "미리보기" : new File(fullPath).getName())
                : displayTitle);
        ...
        btns.add(btnFit); btns.add(btn100); btns.add(btnOut); btns.add(btnIn);
    ...
    private void zoomIn(){ fitMode=false; zoom=Math.min(MAX_ZOOM, zoom*STEP); renderZoom(); }
    private void zoomOut(){ fitMode=false; zoom=Math.max(MIN_ZOOM, zoom/STEP); renderZoom(); }
    private void zoom100(){
        fitMode=false; zoom=1.0; renderZoom();
    ...
    private void toggleFit(){ if (fitMode) zoom100(); else fitToWindow(); }
}
